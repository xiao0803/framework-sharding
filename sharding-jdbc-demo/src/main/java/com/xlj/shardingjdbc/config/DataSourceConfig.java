package com.xlj.shardingjdbc.config;
...
import com.xlj.shardingjdbc.algorithm.ModuloDatabaseShardingAlgorithm;
import com.xlj.shardingjdbc.algorithm.ModuloTableShardingAlgorithm;
import com.xlj.shardingjdbc.costants.DataSourceConstants;
...
public class DataSourceConfig {
    ...
    public DataSource getDataSource() throws SQLException {
        //设置分库映射;注意：分库的规则（见ModuloDatabaseShardingAlgorithm类）与key的后缀有关，如有改动，对应的规则类里面也要改动
        Map<String, DataSource> dataSourceMap = new HashMap<>(2);
        dataSourceMap.put("springboot_0", mybatisDataSource("springboot1"));
    ...
    private DataSource mybatisDataSource(final String dataSourceName) throws SQLException {
        ...
        dataSource.setUrl(String.format(url, dataSourceName));//%s表示字符串类型，因此url里面的%s转换符会被替换
